 ...
 * Tworzy drogę krajową o podanym numerze i przebiegu. Jeśli jakieś miasto lub
 * odcinek drogi nie istnieje, to go tworzy. Jeśli odcinek drogi już istnieje,
 * ale ma wcześniejszy rok budowy lub ostatniego remontu, to modyfikuje ten
 * atrybut odcinka drogi. Za błąd uznajemy, jeśli odcinek drogi już istnieje,
 ...
 * @param cities - wskaźnik na tablicę napisów reprezentujących nazwy miast;
 * @param lengths - wskaźnik na tablicę długości odcinków dróg;
 * @param lastRepairs - wskaźnik na tablicę dat budowy/ostatniego remontu;
 * @param citiesNumber - długość tablicy nazw miast.
 * @return Zwraca @p true, jeśli droga krajowa została poprawnie utworzona.
 * Jeśli podczas dodawania wystąpił błąd (w tym błąd alokacji pamięci) - zwraca
 * @p false.
 */
 ...
 * Usuwa z mapy dróg drogę krajową o podanym numerze, jeśli taka istnieje. Jeśli
 * nie istnieje, niczego nie zmienia.
 * @param map - wskaźnik na strukturę przechowującą mapę dróg;
 * @param routeId - numer drogi krajowej.
 * @return Wartość @p true, jeśli udało się usunąć drogę. Jeśli podana droga
 * krajowa nie istnieje lub podany numer jest niepoprawny, niczego nie zmienia w
 * mapie dróg, dając wynik @p false.
 */
